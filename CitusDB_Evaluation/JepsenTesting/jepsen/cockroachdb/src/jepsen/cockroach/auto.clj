...
(def cockroach-user "User to run cockroachdb as" "postgres")
...
(def db-passwd "")
(def db-port 5432)
(def dbname "postgres") ; will get created automatically

;; Paths
(def working-path "Home directory for cockroach setup" "/usr/pgsql-9.6")
(def cockroach "Cockroach binary" (str working-path "/bin/postgres"))
(def psql "Postgres SQL" (str working-path "/bin/psql"))
(def store-path "Cockroach data dir" "/opt/pgsql/9.6/data")
(def pidfile "Cockroach PID file" "/var/lib/pgsql/9.6/data/postmaster.pid")
...
(defn replication-zone
  ...
  ([name]
   (yaml/parse-string
     (c/cd working-path
           (c/exec cockroach :zone :get name (when insecure #_:--insecure nil))))))
...
(defn start!
  ...
  (c/sudo cockroach-user
          (if (not= "" (try
                         (let[result (c/exec :pgrep :-a :-f (c/lit "[p]ostmaster"))]
                           (info node result)
                           result)
                         (catch RuntimeException e "")))
            (info node "Postgres already running.")
            (do (info node "Starting Postgres...")
                (c/trace (c/exec (runcmd test node
                                         (not= node (jepsen/primary test)))))
                (info node "Postgres started"))))
  :started)

(defn kill!
  "Kills cockroach on node."
  [test node]
  (util/meh (c/su (c/exec :systemctl :stop :postgresql-9.6.service)))
  (info node "Postgres killed.")
  :killed)
